...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        self.cbam  = CBAMLayer(ngf * 8)
        self.down4 = Down(ngf * 8, ngf * 16, norm_layer, use_bias)
 
        model = []
        for i in range(n_blocks):
            model += [ResBlock(ngf * 8, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
    ...
    def forward(self, input):
        ...
        out['d4'] =  self.cbam(out['d3'])
        out['bottle'] = self.resblocks(out['d4'])
